...
// 两个独立进程（armor / power_rune 各一个），主进程只做预处理/后处理，通过
// 本模块通信。共享内存布局与通信协议参考
...
//
// 共享内存 Key 由 config 提供（armor.inference.shm_key / power_rune.inference.shm_key）；
...
// 输入区上限：4 张 640×640×3（armor 最大输入）
constexpr size_t MAX_INPUT_BYTES  = (size_t)4 * 640 * 640 * 3;
// 输出区上限：4 个 [1,25200,22] f32（armor 最大输出，覆盖 power_rune 的 [4,300,102]）
