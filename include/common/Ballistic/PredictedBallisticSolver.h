...
// 默认值）填入，仅为保持返回向量与预测函数目标点列表**下标一一对应**。调用方
// （SequencePredictor）本来就会在选点时跳过这些索引，故行为不变，但省去对应
// 目标点的迭代解算开销。
// ============================================================================
...
public:
    ...
    struct Result {
        ...
        // 该目标点被调用方的 masked_indices 屏蔽：**未做弹道解算**，本结构其余字段
        // （predicted_point / predict_time / gimbal）均为默认值，仅 target_index 有效
        // （= 预测函数返回列表中的原始下标，保持结果向量下标对齐）。
        // masked == true 时 success 恒为 false；但 success == false 也可能是
        // “已解算但失败”（Newton/几何不满足），下游如需区分请判本标志。
        bool   masked = false;
    ...
    // 目标索引 index 是否位于 masked_indices 中（等价于调用方
    // SequencePredictor::Predictor::isIndexMasked，供内部/派生解算器复用）。
    ...
    // （SequencePredictor）完成。masked_indices 中的目标点跳过解算，返回占位符
    // （Result::masked = true），返回向量长度仍与目标点列表一致。
