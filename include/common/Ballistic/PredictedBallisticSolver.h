...
// 对列表中所有目标点，选取"预测点距离当前 muzzle 系原点最近"的目标作为实际使用目标。
//
// 预测序列由调用方（InputController）生成：对 i = 1..n，以
...
public:
    ...
    // 对预测函数返回列表中的每个目标点迭代求解，返回距离当前 muzzle 系原点最近
    // 的目标的预测结果（预测点 / 预测时间 / GimbalSolver 结果）。
