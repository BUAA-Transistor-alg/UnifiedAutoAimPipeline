    ...
    const std::vector<int>& rotation_counts)
{
    ...
    auto func = [predictor_shared, counts](float delta_t) -> std::vector<cv::Vec3f> {
        auto [pos, R] = (*predictor_shared)(delta_t);
        return TargetPositionCalculator::calculate(pos, R, counts);
    };
